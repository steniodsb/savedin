...
import { useRemindersData, ReminderFrequency, ReminderType } from '@/hooks/useRemindersData';
...
export function CreateReminderForm({ onSuccess, onCancel, linkedItem }: ReminderFormProps) {
  const { addReminder } = useRemindersData();
  ...
  const [formData, setFormData] = useState({
    ...
    reminderDate: new Date().toISOString().split('T')[0], // For "once" frequency
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await addReminder({
        ...
        icon: formData.icon,
        type: (linkedItem?.type || 'standalone') as ReminderType,
        ...
        startDate: formData.frequency === 'once' ? formData.reminderDate : formData.startDate,
        isActive: true,
      });
