...
import { ArrowRight, ArrowLeft, Calendar, Tag, Layers, Bell } from 'lucide-react';
...
export function OnboardingFeatureGoals({ onContinue, onBack }: Props) {
  return (
      ...
      <Card className="border-border/10 bg-card/50 backdrop-blur-md shadow-lg overflow-hidden">
        <CardContent className="p-6">
          <motion.div variants={headerVariants} className="text-center mb-6">
            <motion.div className="w-16 h-16 rounded-2xl bg-purple-500/20 flex items-center justify-center mx-auto mb-4" whileHover={{ scale: 1.1, rotate: 5 }} transition={{ type: 'spring', stiffness: 400 }}>
              <Layers className="h-8 w-8 text-purple-500" />
            </motion.div>
            <h2 className="text-2xl font-bold text-foreground mb-2">Organização Completa</h2>
            <p className="text-muted-foreground">Categorize, filtre e visualize tudo de forma clara</p>
          </motion.div>

          <div className="space-y-3 mb-6">
            {[
              { icon: Tag, title: 'Categorias e Subcategorias', desc: 'Organize gastos em categorias hierárquicas', color: 'text-purple-500', bg: 'bg-purple-500/10' },
              { icon: Calendar, title: 'Calendário Financeiro', desc: 'Visualize vencimentos e pagamentos no calendário', color: 'text-blue-500', bg: 'bg-blue-500/10' },
              { icon: Layers, title: 'Múltiplos Ambientes', desc: 'Separe finanças pessoais e de cada empresa', color: 'text-green-500', bg: 'bg-green-500/10' },
              { icon: Bell, title: 'Alertas de Vencimento', desc: 'Nunca perca um pagamento com notificações', color: 'text-amber-500', bg: 'bg-amber-500/10' },
            ].map((f) => (
