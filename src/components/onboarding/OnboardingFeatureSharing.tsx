...
import { ArrowRight, ArrowLeft, Smartphone, Cloud, Lock, Zap } from 'lucide-react';
...
export function OnboardingFeatureSharing({ onContinue, onBack }: Props) {
  return (
      ...
      <Card className="border-border/10 bg-card/50 backdrop-blur-md shadow-lg overflow-hidden">
        <CardContent className="p-6">
          <motion.div variants={headerVariants} className="text-center mb-6">
            <motion.div className="w-16 h-16 rounded-2xl bg-cyan-500/20 flex items-center justify-center mx-auto mb-4" whileHover={{ scale: 1.1, rotate: 5 }} transition={{ type: 'spring', stiffness: 400 }}>
              <Smartphone className="h-8 w-8 text-cyan-500" />
            </motion.div>
            <h2 className="text-2xl font-bold text-foreground mb-2">Acesse em Qualquer Lugar</h2>
            <p className="text-muted-foreground">Seus dados sempre sincronizados e seguros</p>
          </motion.div>

          <div className="space-y-3 mb-6">
            {[
              { icon: Smartphone, title: 'App Progressivo (PWA)', desc: 'Instale no celular como um app nativo', color: 'text-cyan-500', bg: 'bg-cyan-500/10' },
              { icon: Cloud, title: 'Sincronização na Nuvem', desc: 'Seus dados salvos e sincronizados automaticamente', color: 'text-blue-500', bg: 'bg-blue-500/10' },
              { icon: Lock, title: 'Segurança Total', desc: 'Dados criptografados e protegidos', color: 'text-green-500', bg: 'bg-green-500/10' },
              { icon: Zap, title: 'Rápido e Responsivo', desc: 'Interface fluida no desktop e no celular', color: 'text-amber-500', bg: 'bg-amber-500/10' },
            ].map((f) => (
